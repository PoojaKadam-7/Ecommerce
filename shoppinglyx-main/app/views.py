...
class ProductView(View):
    def get(self, request):
        ...
        return render(request,'app/home.html',{'topwears':topwears,'bottomwear':bottomwares,'mobiles':mobiles})
...
def topwear(request, data=None):
    if data == None:
        topwears = Product.objects.filter(category='TW')
    elif data == 'Rodstar' or data == 'max':
        topwears = Product.objects.filter(category='TW').filter(brand=data)
    elif data == 'below':
        topwears = Product.objects.filter(category='TW').filter(discounted_price__lt=350)
    elif data == 'above':
        topwears = Product.objects.filter(category='TW').filter(discounted_price__gt=350)
